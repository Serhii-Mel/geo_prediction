    ...
    def get_video_data_by_video_id(self, video_id: str) -> tuple:
        ...
        while running:
            if iid_did:
                try:
                    # for tests
                    # params['iid'] = 7336372753933992312
                    # params['device_id'] = 7260684190561822547
                    ...
                    running = False
                    with open('temp_info.json',"w",encoding='utf8') as file:
                        json.dump(json_video_data, file, indent=4, ensure_ascii=False)
                    print(params['iid'], params['device_id'])
                except Exception as e:
                    if try_count > 1: # 4 is the max retry per pair
